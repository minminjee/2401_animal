...
public class FileDataUtil {
	...
	@Resource(name="uploadPath")// 이름으로 주입 -> 객체가 필요한다. ->servlet-context.xml
	...
	@ResponseBody   // 어떤 데이터를 포함하여 전송.. 어노테이션.. return타입으로 view지정하지 않고 바로 클라이언트 요청으로 응답.
	...
	//클라이언트의 요청에 서버가 응답하는 타입은 html(view)였지만 파일
	//응답하는 경우가 자료형일 경우 (view 파일이 아닐 경우) @ResponeBody
	...
	public String[] fileUpload(MultipartFile[] file) throws IOException {
		...
		for(int i=0; i < file.length; i++) {
			if(file[i].getOriginalFilename()!="") {  // 실제 file객체가 존재한다면
				...
				UUID uid = UUID.randomUUID();//랜덤문자 구하기 맘에안든다. (중복X)
				String saveName = uid.toString() + "." + originalName.split("\\.")[1];//한글 파일명 처리 때문에...
				// 
//			String[] files = new String[] {saveName}; //형변환  files[0] 파일명이 들어 간다..
				byte[] fileData = file[i].getBytes(); //1byte씩 나누어라?
				
				File target = new File(uploadPath, saveName);// 위치,생성될 파일 이름
	...
	public String fileUpload(MultipartFile file) throws IOException {
		String saveName =null; 
			if(file.getOriginalFilename()!="") {  // 실제 file객체가 존재한다면
				String originalName = file.getOriginalFilename();//확장자가져오기 위해서 전체파일명을 가져옴.
				UUID uid = UUID.randomUUID();//랜덤문자 구하기 맘에안든다. (중복X)
				saveName = uid.toString() + "." + originalName.split("\\.")[1];//한글 파일명 처리 때문에...

				byte[] fileData = file.getBytes(); //1byte씩 나누어라?
