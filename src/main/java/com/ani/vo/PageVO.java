...
public class PageVO {
	...
	private int endNo; // 페이지에서 글 끝번호(튜플의 끝번호)
	//*
	private int perPageNum=10; // 하나의 페이지에 띄울 수 있는 번호
	//*
	private Integer page=1; // 현재 페이지
			// Integer 웹에서 받은 페이지 정보(String)가 없으면 null인데 int는 null을 저장할 수 없다. 오류방지
			//클라이언트가 파라미터를 안보낼 경우 null이 나오는 상황 방지
	//*
